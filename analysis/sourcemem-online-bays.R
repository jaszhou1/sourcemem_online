...
for(h in 1:length(completed.users)){
  ...
  for(j in 1:length(sessions)){
    ...
    ## Extract the required information for each stimuli across the trial types.
    data <- data.frame(matrix(ncol=11,nrow=length(this.session.data$present_trials), dimnames=list(NULL, c("word", "is_sequential",
                                                                                                          "recog_rating","recog_RT","target_angle",
                                                                                                          "response_angle","response_error","source_RT", "valid_RT",
                                                                                                          "block","session"))))
    ## Save session number
...
write.csv(all.data,"~/GitHub/sourcemem_online/analysis/sourcemem_data_bays1.csv", row.names = FALSE)
