# Spatiotemporal_gradient_model

# The multiplicative space * time model, but with a recognition part too. 
# Note that guessing does not change, which is an assumption to revisit at low
# values of recognition confidence
...
# rho: weight of spatial factor (temporal factor is tau, defined as 1-rho)
spacextime_recog_variable_model <- function(params, data){
  ...
  intrusion_weights <- (temporal_similarity^tau) * (spatial_similarity^rho) * intrusion_recognition
  
  # Multiply all intrusion weights with overall intrusion scaling parameter
  intrusion_weights <- gamma*intrusion_weights
  
  if(any(intrusion_weights < 0)){
    print("Negative intrusion weight")
    nLL <- 1e7
    return(nLL)
  }
  ...
  target_weight <- (1 - rowSums(intrusion_weights)) * (1 - beta)
  guess_weight <- (1 - rowSums(intrusion_weights)) * beta
  
  trial_weights <- cbind(target_weight, intrusion_weights, guess_weight)
...
simulate_spacextime_recog_variable_model <- function(participant, data, pest){
