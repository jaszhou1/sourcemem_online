...
# params = [prec, gamma, kappa, lambda_b, lambda_f, beta]
#             1    2      3      4         5    , 6    
...
space_x_time_guess_model <- function(params, data){
  ...
  guess_weight <- 1 - rowSums(intrusion_weights)
  trial_weights <- cbind(intrusion_weights, guess_weight)
  
  # Multiply all weights by 1-beta, the non-guessed responses
  trial_weights <- trial_weights*beta
  
  # Add column for memory weights
  trial_weights <- cbind(target_weight = 1-beta, trial_weights)
...
simulate_space_x_time_guess_model <- function(participant, data, pest){
