# Changes in intrusion probability due to temporal or spatial similarity of non-targets
# affects memory only. However, when an item is not recognised, the weight it would
# be assigned instead goes into guessing.
spacextime_recog_into_guess_model <- function(params, data){
  ...
  # Calculate the target weights before the guessing component
  ...
  target_weight <- target_weight*(1-beta)
  intrusion_weights <- intrusion_weights * (1-beta)
  
  recog_intrusion_weights <- data.frame(matrix(NA, nrow = nrow(intrusion_weights), ncol = ncol(intrusion_weights) + 1))
  for(i in 1:nrow(intrusion_weights)){
    # If an item is unrecognised, in instances where it is an intrusion, reallocate that weight to guess
    this_intrusion_weight <- intrusion_weights[i,]
    this_intrusion_recog <- intrusion_recognition[i,]
    # Find the sum of weights for items not recognised
    this_unrecog_intrusion_weight <- sum(this_intrusion_weight[this_intrusion_recog == 0])
    this_intrusion_weight[this_intrusion_recog == 0] <- 0
    recog_intrusion_weights[i,] <- c(this_intrusion_weight, this_unrecog_intrusion_weight)
  }
  
  trial_weights <- cbind(target_weight, recog_intrusion_weights)
  colnames(trial_weights) <- c("target_weight", "weight_1", "weight_2", "weight_3", "weight_4", 
                               "weight_5", "weight_6", "weight_7", "weight_8", "weight_9", "guess_weight")
  
  trial_weights$guess_weight <- trial_weights$guess_weight + beta
...
simulate_spacextime_recog_into_guess_model <- function(participant, data, pest){
  ...
  for (i in 1:nrow(this_data)){
    ...
    for (j in 1:nSims){
      ...
      if(sim_intrusion_position[length(sim_intrusion_position)]){
        ...
        sim_data[nrow(sim_data)+1,] <- c(word, target_angle, target_position, sim_response, sim_error, no_offset_angles, participant, 'spatiotemporal')
