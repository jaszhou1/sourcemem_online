## spatiotemporal_power_gradient_beta_model.R
...
# params = [prec1, prec2, gamma, kappa, lambda_b, lambda_f, beta, beta_primacy, zeta]
#             1    2       3      4         5    , 6    ,     7,          8      9
...
spatiotemporal_model <- function(params, data){
  ...
  spatial_similarity[,1:9] <- lapply(data[,43:51], shepard_similarity, k = zeta)
  ...
  temporal_similarity <- data[,33:41]
  ...
  intrusion_weights <- temporal_similarity * spatial_similarity
  ...
  trial_weights[!(data$present_trial %in% c(1,2)),] <- trial_weights[!(data$present_trial %in% c(1,2)),] * (1-beta)
  trial_weights[!(data$present_trial %in% c(1,2)), length(trial_weights)] <- beta
...
simulate_spatiotemporal_model <- function(participant, data, pest){
  ...
  block_angles <- cbind(this_data[,7], this_data[,15:23])
